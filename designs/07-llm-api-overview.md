...
```text
base_url + api_key + operation_key + model_id
```
...
模型 spec 可以表达模型能力，但不应该让应用层重新发明一套 provider 转换系统。
...
能力判断可以参考 model spec，但实际请求 shape 由节点上的 `OperationKey` 决定。
...
本项目内部 runtime event、tool abstraction、memory patch 不应该反向进入 `gproxy-protocol`。
...
在这种情况下，Context Assembly 或 LLMNode 仍然发起逻辑 compact 请求，但 shape adapter 把它映射成 Claude 标准 tool 调用。
