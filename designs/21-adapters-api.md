...
LLM 发起的 `propose_memory_change` 使用 `memory_proposal_review` request projection，包含 proposal 的 content、reason、evidence、expected head 与 origin；adapter 只接受恰好覆盖 blocker 的同类 approve/reject decisions，不能在一个 response 混合 tool-call 与 memory-proposal decision。批准/拒绝 proposal、写 immutable decision refs、完成内建 tool call、更新 loop checkpoint、resolve wait 和创建 resume attempt 必须同事务。run terminal 时未启动 proposal tool call 与 blocker 一并 aborted，late wait response 不得恢复 run。
