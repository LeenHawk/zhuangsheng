...
阶段一 `POST /v1/context-presets/{presetId}/preview` 是 published preset 的只读 sample preview。请求显式携带 `versionId?`、`nodeInput`、调用方已经获权且希望测试的 `sampleBindings` 和 `ContextBudgetInput`；缺失的 configured binding 只补一个带 `scope="preview-sample"`、`version="unresolved"` 的空 binding，使 UI 能显示“无 sample candidate”，不得查询 memory/context/artifact 或伪造 runtime read set。响应固定为 `metadata_only`，只返回每个 configured item 的 source/role/token/action/reason、budget totals 和 assembly digest，不回显 input/binding content。该结果证明同一纯 assembly/overflow 算法对这些 sample 的行为，不声称等于某个尚未创建的 NodeInstance；真实 run 仍以其 pinned read set/snapshot 为权威。当前实现只接受诚实标记为 `estimate` 的本地估算，接入版本固定 tokenizer 或显式 provider count 后才能返回 `local` / `provider`。
...
这些内容由前端 compatibility package 转换为本项目的 `ContextItem[]`，后端 context/runtime 不识别酒馆格式，也不会直接保留第三方执行逻辑。导入时：
