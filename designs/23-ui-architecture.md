...
packages/
  ...
  ui-extension-host/ 隔离执行第三方 UI 插件，只接受声明式渲染结果
```
...
UI 插件的 Git 安装、权限确认、版本激活和沙箱契约见 `29-plugin-extension-port.md`。插件不是 core runtime capability，不能直接取得 scheduler、数据库、prompt 或宿主 React/DOM。
...
第三方 UI 插件在 opaque-origin sandbox iframe 内的 Dedicated Worker 执行。iframe CSP 禁止网络和外部资源，worker 可在超时后终止；插件只能返回有深度、节点数、文本量和链接协议上限的声明式 `UiNode`。插件异常、非法输出或超时必须回退庄生原生 renderer。
