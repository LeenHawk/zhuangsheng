    ...
    <div className="mx-auto max-w-7xl pb-24">
      ...
      {run && (
        <>
          ...
          <Card className="mt-4 p-5"><dl className="grid gap-4 text-xs sm:grid-cols-2 lg:grid-cols-4"><Item label="Graph revision" value={run.graphRevisionId} /><Item label="Context / branch" value={`${run.contextId} / ${run.branchId}`} /><Item label="Input commit" value={run.inputCommitId} /><Item label="Deadline" value={new Date(run.deadlineAt).toLocaleString()} /></dl><div className="mt-5 flex flex-wrap gap-2">{(run.status === "running" || run.status === "waiting") && <Button size="compact" variant="secondary" disabled={props.controlPending !== null} onClick={() => void control("interrupt")}><CirclePause className="size-3.5" />暂停</Button>}{run.status === "interrupted" && <Button size="compact" disabled={props.controlPending !== null} onClick={() => void control("resume")}><Play className="size-3.5" />继续</Button>}{!isTerminal(run.status) && (!confirmCancel ? <Button size="compact" variant="ghost" disabled={props.controlPending !== null} onClick={() => setConfirmCancel(true)}><Ban className="size-3.5" />取消运行</Button> : <><Button size="compact" variant="danger" disabled={props.controlPending !== null} onClick={() => void control("cancel")}>确认取消</Button><Button size="compact" variant="ghost" onClick={() => setConfirmCancel(false)}>返回</Button></>)}</div>{props.controlPending && <p className="mt-3 text-xs text-muted">正在提交 {props.controlPending} command…</p>}{props.controlError && <p className="mt-3 text-sm text-danger">{props.controlError}</p>}</Card>
