...
export function buildRolePresetSpec(input: RolePresetInput): JsonObject {
  ...
  return {
    ...
    items: [
      {
        id: "character", name: input.characterName, enabled: true,
        requestedRole: "system", source: { type: "literal", text },
        position: { type: "start" }, order: 0, priority: 100,
        insertionDepth: 0, budget: { required: true }, overflow: null,
      },
      {
        id: "history", name: "Conversation history", enabled: true,
        requestedRole: "context",
        source: { type: "history", bindingId: "history", strategy: { type: "all" } },
        position: { type: "history" }, order: 0, priority: 90,
        insertionDepth: 0, budget: { required: false },
