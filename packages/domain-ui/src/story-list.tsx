...
export function StoryList({ stories, loading, pending, error, onReload, onCreate, onOpen }: StoryListProps) {
    ...
    <div className="mx-auto max-w-6xl pb-24">
      <section className="relative overflow-hidden rounded-3xl border border-default bg-hero px-6 py-10 shadow-panel sm:px-10 lg:py-14">
        <div className="relative z-10 max-w-2xl">
          ...
          <div className="mt-7 flex flex-wrap gap-3"><Button onClick={() => setShowCreate(true)}><Plus className="size-4" />新建故事</Button><Button variant="secondary" onClick={onReload}><RefreshCw className="size-4" />刷新</Button></div>
        </div>
        <div className="story-orb absolute -right-24 -top-28 size-96 rounded-full" aria-hidden="true" />
      </section>
      {showCreate && <Card className="mt-6 p-5"><form onSubmit={submit} className="flex flex-col gap-3 sm:flex-row sm:items-end"><label className="flex-1 text-sm font-semibold">故事名称<span className="mt-1 block text-xs font-normal text-muted">可以稍后修改；留空会创建未命名故事。</span><Input className="mt-2" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="例如：月下档案馆" autoFocus maxLength={200} /></label><div className="flex gap-2"><Button type="button" variant="ghost" onClick={() => setShowCreate(false)}>取消</Button><Button type="submit" disabled={pending}>{pending ? "创建中…" : "创建"}</Button></div></form></Card>}
