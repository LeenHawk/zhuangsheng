  ...
  return (
    <div className="space-y-4">
      ...
      {props.draft && props.status !== "loading" && <>
        <Card className="p-4 md:hidden"><p className="text-sm font-semibold">移动端只读结构列表</p><p className="mt-1 text-xs text-warning">完整 Graph 编辑需要至少 640px 可用宽度；此处不会缩小画布伪装支持。</p><div className="mt-3 space-y-2">{props.projection?.nodes.map((node) => <div key={node.id} className="rounded-xl bg-elevated p-3"><p className="font-mono text-xs font-semibold">{node.name ?? node.id}</p><p className="mt-1 text-[11px] text-muted">{node.kind} · {node.id}</p></div>)}</div></Card>
        <div className="hidden gap-4 md:grid xl:grid-cols-[minmax(0,1.6fr)_minmax(360px,0.8fr)]">
